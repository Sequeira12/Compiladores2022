...
void verifica(no raiz){
    if(raiz){
        if(strcmp(raiz->s_type,"Program")==0) {if(raiz->filho){nova_classe(raiz->filho->valor);}}
        ...
        if(raiz->filho){
        insere(raiz->filho,NULL,NULL,"Class");
        }
...
void verifica_method_decl(no node){
    char *tipo, *valor, *parametros,**array_de_parametros;
    if(node->filho){
    tipo = verifica_type(node->filho->s_type);
    }
    if(node->filho->irmao){
     valor = (char*)strdup(node->filho->irmao->valor);
    }
     if(node->filho->irmao){
    parametros = verifica_method_params(node->filho->irmao->irmao);
    array_de_parametros = verifica_array_method_params(node->filho->irmao->irmao);
     }
    ...
    if(node){
        ...
        if(strcmp(node->s_type,"While")==0){
            ...
            if(strcmp(node->filho->s_type,"Xor")==0){
                if(node->filho->filho && node->filho->filho->irmao){
       ...
       if(strcmp(node->s_type,"RealLit")==0){
        ...
        if(strcmp(node->s_type, "Assign")==0){
          ...
          if(strcmp(node->filho->irmao->s_type, "Minus")==0 || strcmp(node->filho->irmao->s_type, "Plus")==0){
            ...
            if(strcmp(node->filho->irmao->s_type,"Lshift")==0){
                if(node->filho->irmao->filho && node->filho->irmao->filho->irmao){
        ...
        if(strcmp(node->s_type,"Call")==0){
            ...
            tabela noo = procura_tab(tab);
            if(node->pai){
            if(strcmp(node->pai->s_type, "Return")==0 && strcmp(node->id, noo->tab->s_type)!=0){
                printf("Line %s, col %s: Incompatible type %s in return statement\n",node->line,
